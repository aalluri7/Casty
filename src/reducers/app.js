import { CACHE_PODCAST,SET_FEED_URL,SET_TOP_PODCASTS,SET_URL_DATA, SET_CURRENT_VIEW,SET_PODCAST, SET_URL, SET_CURRENT_EPISODE, ADD_PODCAST, NORMALIZE_PODCAST_DATA,SET_ACTIVE_PODCAST,SET_SEARCH,SET_SEARCH_RESULTS,SET_PAGE_VIEW } from '../constants/ActionTypes';
...
function feedNormalizer(feed) {
    let imgurl = getvalue(feed.image,function(x,y){return x=='url'||x=='href'});
    console.log(imgurl);
...
export default function app(state = initialState, action) {
  ...
  case CACHE_PODCAST:
    return Object.assign({}, state, { cachedPodcasts: Object.assign({}, state.cachedPodcasts, { [action.feedUrl]: action.feed }) });
  case SET_SEARCH:
  ...
  case ADD_PODCAST:
    return Object.assign({}, state, {subscribedPodcasts:Object.assign({}, state.subscribedPodcasts, { [action.podcastName]: {name:action.podcastName,feedUrl:action.feedUrl} })} );
  case NORMALIZE_PODCAST_DATA:
    return Object.assign({}, state, { feed: feedNormalizer(state.feed) });
  case SET_ACTIVE_PODCAST:
    return Object.assign({}, state, { feed: state.subscribedPodcasts[action.podcastName].feed });
