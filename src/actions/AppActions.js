import { CACHE_PODCAST,SET_FEED_URL,SET_TOP_PODCASTS,SET_PODCAST, SET_URL, SET_CURRENT_EPISODE,ADD_PODCAST,NORMALIZE_PODCAST_DATA,SET_ACTIVE_PODCAST,SET_SEARCH,SET_SEARCH_RESULTS,SET_CURRENT_VIEW,SET_URL_DATA } from '../constants/ActionTypes';
...
export function subscribePodcastById(id) {
  ...
  return (dispatch, getState) => {
    ...
    $.ajax({
            ...
            dataType: "jsonp",
            success: function(data){
            dispatch(addPodcast(data.results[0].trackName,data.results[0].feedUrl));
            }
...
export function fetchPodcast(rssUrl) {
  ...
  return (dispatch, getState) => {
    ...
    let yqlUrl = 'http://query.yahooapis.com/v1/public/yql?q='+encodeURIComponent('select * from xml where url="'+rssUrl+'"') + '&format=json';
